      ...
      ["application", "conditions", "delivery", "receiving"].forEach(
        (cardKey) => {
          ...
          if (cardElement && numElement) {
            ...
            // Ширина numDigit примерно 13.6% от ширины num (5.14rem / 37.67rem)
            // position 0 -> left: 6.8% (половина ширины цифры, чтобы цифра не выходила за левый край)
            // position 0.5 -> left: 50% (центр)
            // position 1 -> left: 93.2% (100% - 6.8%, чтобы цифра не выходила за правый край)
            const digitWidthPercent = 13.6; // Примерная ширина цифры в процентах
        ...
        >
          <div className="num" ref={(el) => (numRefs.current.application = el)}>
            <span
              className="numDigit"
              style={{
                left: `${digitPositions.application * 100}%`,
        ...
        >
          <div className="num" ref={(el) => (numRefs.current.conditions = el)}>
            <span
              className="numDigit"
              style={{
                left: `${digitPositions.conditions * 100}%`,
                transform: "translateX(-50%)",
              }}
            >
              02
        ...
        >
          <div className="num" ref={(el) => (numRefs.current.delivery = el)}>
            <span
              className="numDigit"
              style={{
                left: `${digitPositions.delivery * 100}%`,
                transform: "translateX(-50%)",
              }}
            >
              03
        ...
        >
          <div className="num" ref={(el) => (numRefs.current.receiving = el)}>
            <span
              className="numDigit"
              style={{
                left: `${digitPositions.receiving * 100}%`,
                transform: "translateX(-50%)",
              }}
            >
              04
