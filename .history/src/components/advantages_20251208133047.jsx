    ...
    const observerCallback = (entries, pairIndex) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          // Активируем обе карточки в паре с небольшой задержкой для эффекта
    ...
    const setupObservers = () => {
      cardPairs.forEach((pair, pairIndex) => {
        ...
        if (card1Element && card2Element) {
          // Проверяем, видна ли хотя бы одна карточка из пары при загрузке
          const rect1 = card1Element.getBoundingClientRect();
          const rect2 = card2Element.getBoundingClientRect();
          const isVisible =
            (rect1.top < window.innerHeight + 200 && rect1.bottom > -200) ||
            (rect2.top < window.innerHeight + 200 && rect2.bottom > -200);

          if (isVisible) {
            setVisibleCards((prev) => ({ ...prev, [card1Key]: true }));
            setTimeout(() => {
              setVisibleCards((prev) => ({ ...prev, [card2Key]: true }));
            }, 100);
          }

          // Наблюдаем за первой карточкой пары
