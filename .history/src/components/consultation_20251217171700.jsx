...
export default function Consultation() {
  ...
  useEffect(() => {
    ...
    if (consultationElement) {
      consultationElement.addEventListener("mousemove", handleMouseMove);
      return () => {
        consultationElement.removeEventListener("mousemove", handleMouseMove);
      };
    }
  }, []);

  const translateX = mousePosition.x * 1.71; // Максимальное смещение 1.71rem
  const translateY = mousePosition.y * 1.71; // Максимальное смещение 1.71rem

  return (
    <div className="consultationCont" ref={consultationRef}>
      <Image
