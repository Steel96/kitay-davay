"use client";
import { useEffect } from "react";

export default function YandexMap() {
  useEffect(() => {
    if (typeof window === "undefined") return;

    const script = document.createElement("script");
    script.src =
      "https://api-maps.yandex.ru/v3/?apikey=bdcfda32-d4a0-48b1-bb6d-7bf06f6a2adb&lang=ru_RU";
    script.async = true;

    const onLoadHandler = async () => {
      if (!window.ymaps3) {
        console.error("Yandex Maps API did not load correctly.");
        return;
      }

      const ymaps3 = window.ymaps3;

      try {
        const { YMap, YMapDefaultSchemeLayer, YMapDefaultFeaturesLayer } =
          await ymaps3.import("@yandex/ymaps3");
        await ymaps3.import("@yandex/ymaps3-default-ui-theme");

        const map = new YMap(
          document.getElementById("yandex-map"),
          {
            location: {
              center: [37.57383, 55.7352],
              zoom: 15,
            },
          },
          [new YMapDefaultSchemeLayer(), new YMapDefaultFeaturesLayer()]
        );

        const { YMapMarker } = await ymaps3.import("@yandex/ymaps3-markers");

        const markerElement = document.createElement("div");
        markerElement.style.width = "20px";
        markerElement.style.height = "20px";
        markerElement.style.background = "red";
        markerElement.style.borderRadius = "50%";
        markerElement.style.border = "3px solid white";
        markerElement.style.boxShadow = "0 0 5px rgba(0,0,0,0.3)";

        const marker = new YMapMarker(
          { coordinates: [37.57383, 55.7352] },
          markerElement
        );

        map.addChild(marker);
      } catch (error) {
        console.error("Error loading Yandex Maps modules:", error);
      }
    };

    script.addEventListener("load", onLoadHandler);
    document.body.appendChild(script);

    return () => {
      const elem = document.getElementById("yandex-map");
      if (elem) elem.innerHTML = "";

      // Удаляем скрипт, чтобы избежать повторной загрузки при размонтировании/монтировании
      script.removeEventListener("load", onLoadHandler);
      if (document.body.contains(script)) {
        document.body.removeChild(script);
      }
    };
  }, []);

  return (
    <div
      id="yandex-map"
      style={{
        width: "100%",
        height: "400px",
        borderRadius: "12px",
        overflow: "hidden",
      }}
    />
  );
}
