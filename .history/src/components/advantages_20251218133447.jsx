...
export default function Advantages() {
  ...
  useEffect(() => {
    ...
    const observerCallback = (entries, pairIndex) => {
      entries.forEach((entry) => {
        ...
        // Проверяем, что элемент действительно виден достаточно хорошо
        const isVisible =
          entry.isIntersecting && entry.intersectionRatio >= 0.2;

        if (isVisible) {
          // Активируем обе карточки в паре с задержкой для эффекта
          // Проверяем, что карточки еще не видимы, чтобы избежать повторных анимаций
          setVisibleCards((prev) => {
            if (!prev[card1] && !prev[card2]) {
          ...
          setTimeout(() => {
            setVisibleCards((prev) => {
              if (prev[card1] && !prev[card2]) {
                return { ...prev, [card2]: true };
              }
              return prev;
            });
          }, 150);
        } else {
          // Сбрасываем состояние только если элемент полностью вышел из viewport
          // Используем более строгое условие, чтобы избежать дергания
    ...
    const setupObservers = () => {
      cardPairs.forEach((pair, pairIndex) => {
        ...
        if (card1Element) {
          // Наблюдаем только за первой карточкой пары, чтобы избежать двойных срабатываний
  ...
  return (
    <div id="advantages" className="advantagesCont">
      ...
      <div className="advantagesContainer">
        ...
        >
          <span className="advantagesText4">
            У нас собственный ВЭД контракт. Поэтому таможенные платежи ниже, чем
            у других на 10%.
          </span>
          <button className="advantagesBtn" onClick={() => setOpen(true)}>
            ПОДРОБНЕЕ
